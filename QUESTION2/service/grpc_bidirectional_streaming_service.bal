    ...
    remote function chat(ChatStringCaller caller,
        ...
        do {
            _ = check from ChatMessage chatMsg in clientStream
                do {
                    checkpanic caller->sendString(string `> Title ${chatMsg.title}: Author ${chatMsg.author_1 } :Author number 2  ${chatMsg. optional_author_2}: Shelve Location ${chatMsg.location_of_the_book_in_library }: Status ${chatMsg.ISBN}: ${chatMsg.status }`);
                };
